...
function calculateLevel(totalExp: number): { level: number; currentExp: number; expToNextLevel: number } {
  let level = 1;
  while (true) {
 ...
 */
export const addUserExp = functions.https.onCall(async (data, context) => {
...
export const checkAchievements = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    return db.runTransaction(async (transaction) => {
      ...
      const unlockedIds: string[] = doc.exists ? (doc.data()?.unlockedIds || []) : [];
      ...
      if (newlyUnlocked.length > 0) {
        ...
        if (totalExpReward > 0) {
          const userRef = db.collection('users').doc(uid);
          const userDoc = await transaction.get(userRef);
          const currentExp = userDoc.data()?.totalExp || 0;
 ...
 */
export const generateQuestWithAI = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
